...
def validate(
    ...
    device = torch.device("cpu")  # torch.device('cuda:{}'.format(dist.get_rank()))
    model.eval()
    ...
    with torch.no_grad():
        ...
        features_s = (
            model.module.extract_features(spprt_imgs)
            .detach()
            .view((args.num_classes_val, args.shot, c, h, w))
        )
        gt_s = s_label.view(
    ...
    results.append(
        f"%-30s \t %.2f" % ("Weighted-sum of Base-and-Novel mIoU", wght_sum_mIoU * 100)
    )
    results.append(
